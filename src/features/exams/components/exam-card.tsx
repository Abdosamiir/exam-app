...
const ExamCard = ({ exam }: ExamCardProps) => {
  return (
    <Link
      href={`/exams/${exam.id}`}
      className="group border-2 flex-1 flex-col md:flex-row bg-primary/5 p-5 shadow-sm flex w-full gap-3 hover:border-dashed hover:border-2 hover:border-primary/30 hover:shadow-md transition-shadow"
    >
      {exam.image && (
        <Image
          src={exam.image}
          alt={exam.title}
          className="h-25 w-25 border border-primary/40 p-4 bg-primary/10   object-cover"
          width={100}
          height={200}
        />
      )}
