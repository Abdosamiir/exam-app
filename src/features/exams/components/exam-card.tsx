...
const ExamCard = ({ exam }: ExamCardProps) => {
  return (
    <Link
      href={`/exams/${exam.id}`}
      className="group border-2 flex-1 bg-blue-50 p-5 shadow-sm flex w-full gap-3 hover:border-dashed hover:border-2 hover:border-blue-200 hover:shadow-md transition-shadow"
    >
      {exam.image && (
        <Image
          src={exam.image}
      ...
      <div className="flex flex-col gap-1 w-full">
        ...
        <Button className="w-25 px-4 mt-5 self-end rounded-none uppercase flex items-center gap-2 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
